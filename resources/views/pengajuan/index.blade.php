...
@section('contents')
    <main id="main" class="main">
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-striped" id="pendaftaranTable">
                                ...
                                @php
                                    ...
                                    @php
                                        $konklusiIsFilled = !empty($pendaftaran->pn_konklusi);
                                    @endphp
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($konklusiIsFilled)
                                                {{ $pendaftaran->pn_konklusi }}
                                            @else
                                                <button class="btn btn-success edit-konklusi" data-toggle="modal" data-target="#konklusiModal" data-id="{{ $pendaftaran->id }}">
                                                    Isi Konklusi
                                                </button>
                                            @endif
                                        </td>
        ...
        <!-- Modal Konklusi -->
                <div class="modal fade" id="konklusiModal" tabindex="-1" role="dialog" aria-labelledby="konklusiModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="konklusiModalLabel">Isi Konklusi</h5>
                            </div>
                            <div class="modal-body">
                            <form action="{{ route('pengajuan.konklusi', ['id' => $pendaftaran->id]) }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="pendaftaran_id" id="konklusiPendaftaranId">
                                    <div class="form-group">
                                        <label for="konklusiTextarea">Hasil Konklusi:</label>
                                        <textarea class="form-control" id="konklusiTextarea" name="pn_konklusi" rows="3"></textarea>
                                    </div>
                                    <button type="submit" class="btn btn-primary">Simpan Konklusi</button>
                                </form>
                            </div>
                        </div>
                    </div>
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function () {
                        const editKonklusiBtns = document.querySelectorAll('.edit-konklusi');

                        editKonklusiBtns.forEach(btn => {
                            btn.addEventListener('click', function () {
                                const pendaftaranId = this.getAttribute('data-id');
                                document.getElementById('konklusiPendaftaranId').value = pendaftaranId;
                                $('#konklusiModal').modal('show');
                            });
                        });
                    });
                </script>
